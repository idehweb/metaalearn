...
class CreateForm extends React.Component {
  ...
  render() {
    let REF = this;
    let { buttons, t, data, id, fields, texts, rating } = this.props;
    let { val } = this.state;

    console.log('fields tx', fields);
    console.log('rating tx', rating);
    return [
      rating && rating.map((field, index) => {
        return (
          <div className={'rate-wrapper'}>
            <Rating key={index} name="half-rating"
              onChange={(e) => {
                field.onChange(e.target.value);
                console.log('rating log', e.target.value)
          ...
          if (field.type && field.type === 'title') {
            return (
              <Col key={index} sm={field.size.sm} lg={field.size.lg}>
                <span className="kjhghjk">{field.title}</span>
                <div>
                  <div
                    id={'defvgbnb'}
