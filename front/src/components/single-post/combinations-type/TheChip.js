...
const theChip = (props) => {
  ...
  let choosed = combinations[0],
    doptions = combinations[0].options,
    lessPrice = combinations[0].salePrice || combinations[0].price,
    dstock = combinations[0].in_stock;
  ...
  let inS =
    theCombination.in_stock == '0' || theCombination.in_stock == null
      ? false
      : true;
  // if (!inS && !single)
  //   return;
  ...
  return [
    <div className={' mt-5 the-chip row'}>
      <label className={'the-label-inline bigger'}>
        {t('please choose combination') + ':'}
      </label>
    </div>,
    <div className={' mt-2 the-chip row'}>
      ...
      <div className={'col-md-4'}>
        {Boolean(theCombination) && (
          <div className={'the-option-price text-center'}>
            <div className={'gfd'}>
              {/*{theCombination.price}/*/}
              <Theprice
                className={
                  'single single-let ' +
                  theCombination.salePrice +
                  ' - ' +
                  theCombination.price
                }
                ...
                <div className={'the-option-actions ' + inS}>
                  <AddToCardButton
                    item={{
                      ...
                      title: {
                        [lan]:
                          title[lan] + ' - ' + handleTitles(theCombination),
                      },
                      // mainTitle: title,
                      photos: photos,
                      weight: weight,
                ...
                <div className={'the-option-actions ' + inS}>
                  <AddToCardButton
                    item={{
                      ...
                      photos: photos,
                      weight: Number(theCombination.weight),
                      single: true,
                      in_stock: inS,
                      quantity: parseInt(theCombination.quantity),
                      price: theCombination.price,
        ...
        opt.values.map((val, j) => {
          return (
            <Chip
              key={j}
              icon={
                actives && actives[opt.name] === val.name ? (
              ...
              className={
                actives && actives[opt.name] === val.name ? 'active' : ''
              }
              label={val.name}
              onClick={(e) => {
                onClick(val, j);
