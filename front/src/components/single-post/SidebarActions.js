...
class SidebarActions extends React.PureComponent {
  ...
  render() {
    ...
    let { requireWarranty,t, updatedAt, countryChoosed, type, _id, firstCategory, in_stock, quantity, secondCategory, thirdCategory, title, photos, weight, price, salePrice, options, combinations, method, single = true } = this.props;
    console.log('sidbarcard', this.props)
    ...
    return (
      [<div key={0}>
        {type == "variable" && combinations && <Col lg={12} md={12} sm={12} xs={12}>
          <Row>
            {Boolean(!isSSR && combinations.length>0) && <TheChip requireWarranty={requireWarranty} _id={_id} weight={weight} title={title} photos={photos} options={options} single={single} method={method} combinations={combinations} t={t} />}
            {Boolean(isSSR && combinations.length>0) && <TheList _id={_id} title={title} photos={photos} options={options} weight={weight} single={single} method={method} combinations={combinations} t={t}/>}
