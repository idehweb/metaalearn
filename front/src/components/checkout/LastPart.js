...
const LastPart = (props) => {
  ...
  console.log("t last part and the params", t, theParams)
  ...
  //   }
  // },[]);
  const {_id, onNext, onPlaceOrder, onPrev, prev = 'show'} = props;
        ...
        <Col lg="12">
          ...
          <ListGroup flush className={'card-add checkout'}>
                ...
                <ListGroupItem
                  ...
                  <div className={'flex-2 pl-2'}>
                    {item.price && !item.salePrice && (
                      <div className={'prc'}>
                        {returnPrice(item.price * item.count)}
                      </div>
                    )}
                    {item.price && item.salePrice && (
                      <div className={'prc'}>
                        {returnPrice(item.salePrice * item.count)}
                        {/*<del*/}
            ...
            <ListGroupItem className={'d-flex px-3 border-0 '}>
              {[
                ...
                <div className={'flex-1 textAlignRight'} key={'xo3'}>
                  {sum && <div className={'ttl '}>{returnPrice(sum)}</div>}
                </div>,
              ]}
            </ListGroupItem>
            <ListGroupItem className={'d-flex px-3 border-0 '}>
              {[
                <div className={'flex-1'} key={'xo2'}>
                  <div className={'ttl'}>{t('deliveryPrice') + ': '}</div>
                </div>,
                <div className={'flex-1 textAlignRight'} key={'xo3'}>
                  {deliveryPrice && <div className={'ttl '}>{returnPrice(deliveryPrice)}</div>}
                </div>,
              ]}
            </ListGroupItem>
              ...
              <ListGroupItem className={'d-flex px-3 border-0 '}>
                {[
                  ...
                  <div key={'xo5'} className={'flex-1 textAlignRight'}>
                    <div className={'ttl '}>
                      {taxAmount + '%'} -{' '}
                      {returnTaxAmount(taxAmount) + ' ' + t(currency)}
                    </div>
                  </div>,
                ]}
              </ListGroupItem>
            ...
            <ListGroupItem className={'d-flex px-3 border-0 '}>
              {[
                ...
                <div key={'xo7'} className={'flex-1 textAlignRight'}>
                  <div className={'ttl '}>
                    {amount &&
                    returnAmount(amount, taxAmount) + ' ' + t(currency)}
                  </div>
                </div>,
              ]}
            </ListGroupItem>
            ...
            <ListGroupItem className={'d-flex px-3 border-0 '}>
              {[
                <div className={'flex-1'} key={'xo6'}>
                  <div className={'ttl'}>{t('first & last name') + ': '}</div>
                </div>,
                <div key={'xo7'} className={'flex-1 textAlignRight'}>
                  <div className={'ttl '}>
            ...
            <ListGroupItem className={'d-flex px-3 border-0 '}>
              {[
                <div className={'flex-1'} key={'xo6'}>
                  <div className={'ttl'}>{t('phone number') + ': '}</div>
                </div>,
                <div key={'xo7'} className={'flex-1 textAlignRight'}>
                  <div className={'ttl '}>{user && user.phoneNumber}</div>
                </div>,
              ]}
            </ListGroupItem>
            <ListGroupItem className={'d-flex px-3 border-0 '}>
              {user && user.internationalCode && [
                <div className={'flex-1'} key={'xo6'}>
                  <div className={'ttl'}>{t('internationalCode') + ': '}</div>
                </div>,
                <div key={'xo7'} className={'flex-1 textAlignRight'}>
                  <div className={'ttl '}>{user.internationalCode}</div>
                </div>,
              ]}
            </ListGroupItem>
            <ListGroupItem className={'d-flex px-3 border-0 '}>
              ...
              <GetGateways
                setPaymentMethod={props.theParams.setPaymentMethod}
              />
              {/*</ListGroup>*/}
            </ListGroupItem>
              ...
              <ListGroupItem className={'d-flex px-3 border-0 '}>
                {[
                  <div className={'flex-1'} key={'xo8'}>
                    <div className={'ttl'}>
                      {'سقف پرداخت اینترنتی 50 میلیون تومان است.'}
                    </div>
                    <div className={'ttl'}>
                      {'باید در چند مرحله پرداخت کنید:'}
                    </div>
                  </div>,
